...
"""Training script for cell classification neural network models."""
...
class Existing(str, Enum):
    """Enum for handling existing output files."""
...
class LrScheduler(str, Enum):
    """Enum for supported learning rate schedulers."""
...
class StoppingCriteria:
    """Early stopping criteria based on minimum improvement thresholds."""
    ...
    def __init__(
        ...
        Args:
            patience: Number of times to allow no improvement before stopping.
            min_delta: Minimum absolute change counted as improvement.
            min_rel_delta: Minimum relative change counted as improvement.
...
def run(args: argparse.Namespace) -> None:
    """Train a cell classification model using the provided arguments."""
    import logging
    import os
    import shutil
    import time
    ...
    for i in range(args.epochs):
        epoch += 1
        train_loss, val_loss, train_acc, val_acc, y_true, y_pred = train_epoch(
            model,
            train_loader,
            validation_loader,
            loss_fn,
        ...
        pr, re, f1, _ = precision_recall_fscore_support(
            y_true_np, y_pred_np, zero_division=np.nan, average="weighted"
        )
        better = False
        if f1 > best_f1:
            best_f1 = f1
        ...
        if testing_loader and (i + 1) % test_interval == 0:
            ...
            *test_stats, _ = precision_recall_fscore_support(
            ...
            test_stats.append((y_true_np == y_pred_np).sum() / len(y_true_np))
    ...
    if testing_loader and "test_f1" not in checkpoint:
        ...
        *test_stats, _ = precision_recall_fscore_support(
        ...
        checkpoint["test_acc"] = (y_true_np == y_pred_np).sum() / len(
            y_true_np
        )
