...
- Annotated `.npy` crop files produced by the
  [Training Widget](../../napari/user-guide/training_widget.html), stored in a single
...
- [Weights & Biases](installation.qmd#weights-biases) credentials configured.
...
[Model Guide](models.qmd) for recommended architectures for small cell images.
...
For SLURM on the Artemis cluster see the [SLURM guide](slurm.qmd).
...
[pipeline configuration](../../user-guide/installation.html#configure) for details.
