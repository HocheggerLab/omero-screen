---
title: "omero-train CLI"
---

::: {.callout-note}
The full option detail for every `omero-train` command — flags, defaults,
arguments and types — is generated directly from the code in the
[CLI reference](../reference/cli/index.html). This page covers what the tool is
for and how it is used in practice; go to the generated pages for the exact
switches.
:::
...
|---------|-------------|
| [`list`](../reference/cli/list.html) | Show all classifiers with summary statistics |
| [`stats`](../reference/cli/stats.html) | Show detailed statistics for one classifier |
| [`export`](../reference/cli/export.html) | Export annotations to CSV, JSON, or Parquet |
| [`delete`](../reference/cli/delete.html) | Remove a classifier and its sessions |
| [`migrate`](../reference/cli/migrate.html) | Import existing NPY files into the database (for legacy data) |
...
You can choose the output format, set the output path, and restrict the export
to a single plate or well — see [`omero-train export`](../reference/cli/export.html)
for the option list and defaults.
...
You can skip the confirmation prompt, or delete only the sessions belonging to
one plate — see [`omero-train delete`](../reference/cli/delete.html) for the
option list.
...
You can point the scan at a different root directory, or preview the import
without writing anything — see [`omero-train migrate`](../reference/cli/migrate.html)
for the option list and defaults.
