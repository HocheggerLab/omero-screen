...
class ImageClassifier:
    ...
    def process_images(
        ...
        # Extract the timepoint from the dataframe and process with the timepoint from the mask and image.
        df_final = pd.DataFrame()
        frames = mask.shape[0]
        for t in range(frames):
            df_t = original_image_df[original_image_df["timepoint"] == t]
            df = self._process_images(df_t, mask, t)
    ...
    def _process_images(
        ...
        Args:
            ...
            t: Time point
        Returns:
            Dataframe supplemented with the classification column.
        """
        ...
        # TODO: handle multiple time input.
        # The cytoplasm IDs reset to 1 at each timepoint.
        # Extract the timepoint from the dataframe and process with the timepoint from the mask and image.
        # Add an output loop to call this method with the timepoint and build up a new dataframe with the
        # results.
        ...
        for start in range(0, total, step):
            ...
            for i in range(start, stop):
                ...
                # Crop mask
                cropped_mask = self._crop(mask, t, x0, y0, x1, y1).copy()
