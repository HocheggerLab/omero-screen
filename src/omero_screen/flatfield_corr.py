"""This module generates flatfield correction masks for a given plate and uploads them to the OMERO server.

The main function, flatfieldcorr, computes flatfield correction masks for
each channel in the plate. The generated masks are stored in the associated
dataset within the Screens project on the OMERO server. Quality control
examples are saved as PDF attachments to the dataset. The flatfieldcorr
function returns a dictionary mapping channel names to their corresponding
...
def upload_images(
    ...
    def plane_gen() -> Generator[npt.NDArray[Any]]:
        ...
        Yields:
            npt.NDArray[Any]: Each plane in the array_list.
        """
...
def load_image_to_dict(
    ...
    """Loads an image from the OMERO server and returns a channel-to-array dict.

    Converts the image to a dictionary with channel names as keys and
    corresponding image arrays as values.
...
def random_timgs(image_array: npt.NDArray[Any]) -> list[npt.NDArray[Any]]:
    """Selects up to 10 random timepoint images from a 5D image array.

    Args:
        image_array: A numpy array of images, where the first dimension
            represents timepoints.

    Returns:
        A list of numpy arrays, each corresponding to a selected timepoint.
    """
